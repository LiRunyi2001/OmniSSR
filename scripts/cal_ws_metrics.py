...
def cal_ws_metrics(src_gt, src_sr, metrics=None, print_avg=True, print_details=True, suffix=".png"):
    '''
    ...
    pths_gt = sorted(gb.glob(gb.escape(src_gt)+'/*'+suffix))
    pths_sr = sorted(gb.glob(gb.escape(src_sr)+'/*'+suffix))

    filtered_pths_sr = []
    for filename in pths_sr:
        if 'lq' not in filename:
            filtered_pths_sr.append(filename)
    pths_sr = filtered_pths_sr
    ...
    print("num of gt:", len(pths_gt))
    print("num of sr:", len(pths_sr))
    ...
    for i, (pth_gt, pth_sr) in enumerate(zip(pths_gt, pths_sr)):
        ...
        if 'wspsnr' in metrics:
            _output = calculate_psnr_ws(img_gt, img_sr, crop_border=0,test_y_channel=False)
        ...
        if 'wsssim' in metrics:
            _output = calculate_ssim_ws(img_gt, img_sr, crop_border=0,test_y_channel=False)
