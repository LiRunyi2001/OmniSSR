...
@DATASET_REGISTRY.register()
class MultiSingleImageDataset(data.Dataset):
    """Read only lq/gt images in the test phase.
    ...
    def __init__(self, opt):
        super(MultiSingleImageDataset, self).__init__()
        self.opt = opt
        # file client (io backend)
        self.file_client = None
        self.io_backend_opt = opt['io_backend']
    ...
    def __getitem__(self, index):
        ...
        gt_path = self.gt_paths[index]
        img_bytes_gt = self.file_client.get(gt_path, 'gt')
        img_gt = imfrombytes(img_bytes_gt, float32=True)  # float32, [0 ~ 1]

        if self.opt['phase'] == 'train':
            gt_size = self.opt['gt_size']
            # random crop
            img_gt= single_random_crop(img_gt, gt_size, gt_path)
            # flip, rotation
...
def single_random_crop(imgs, patch_size, gt_path=None):
    """Single random crop. Support Numpy array and Tensor inputs.

    It crops a square patch from input image randomly. If the input image is
    smaller than the patch size, return the original image.

    Args:
        img (ndarray | Tensor): Image to be cropped. If the input is a Tensor,
            it will be transformed to a Numpy array.
        patch_size (int): Crop patch size.
