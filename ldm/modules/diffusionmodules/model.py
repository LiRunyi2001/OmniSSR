...
class MemoryEfficientAttnBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        h_ = (
            out.permute(0,2,1)
            .unsqueeze(3)
            .reshape(b, c, h, w).contiguous() # add .contiguous() by sxh
        )
        ...
        for i_level in range(self.num_resolutions):
            ...
            if return_fea:
                if i_level==1 or i_level==2:
                    fea_list.append(h)
            if i_level != self.num_resolutions-1:
                hs.append(self.down[i_level].downsample(hs[-1]))   # 基于 conv 的下采样
...
class Decoder(nn.Module):
    ...
    def forward(self, z):
        ...
        # z to block_in
        h = self.conv_in(z.contiguous())

        # middle
        h = self.mid.block_1(h.contiguous(), temb)
        h = self.mid.attn_1(h)
        h = self.mid.block_2(h.contiguous(), temb)
...
class Decoder_Mix(nn.Module):
    def __init__(self, *, ch, out_ch, ch_mult=(1,2,4,8), num_res_blocks,
        ...
        # upsampling
        self.up = nn.ModuleList()
        for i_level in reversed(range(self.num_resolutions)):  # i_level in [3,2,1,0]
            block = nn.ModuleList()
            attn = nn.ModuleList()
            block_out = ch*ch_mult[i_level]
