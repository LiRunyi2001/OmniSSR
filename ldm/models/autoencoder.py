...
class AutoencoderKL(pl.LightningModule):
    ...
    def init_from_ckpt(self, path, ignore_keys=list(), only_model=False):
        ...
        for k in keys:
            if 'first_stage_model' in k:
                sd[k[18:]] = sd[k]  # 'first_stage_model.encoder.conv_in.weight' -> 'encoder.conv_in.weight'
            for ik in ignore_keys:
                if k.startswith(ik):
                    print("Deleting key {} from state_dict.".format(k))
                    del sd[k]
    ...
    def encode(self, x, return_encfea=False):
        h = self.encoder(x)  # [1, 8, 128, 256]
        moments = self.quant_conv(h)  # [1, 8, 128, 256]
...
class AutoencoderKLPanoV1(pl.LightningModule):
    def __init__(self,
                 ddconfig,
                 lossconfig,
                 embed_dim,
        ...
        if ckpt_path is not None:
            missing_list, unexpected_list = self.init_from_ckpt(ckpt_path, ignore_keys=ignore_keys)
        else:
            missing_list, unexpected_list = [], []
        ...
        # only train the decode params: decoder and the post_quant_conv
        for name, param in self.named_parameters():
            if 'decoder' in name:
                param.requires_grad = True
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        ...
        inputs = self.get_input(batch, self.image_key)
        reconstructions, posterior = self(inputs)
        self.log("global_step", int(self.global_step), prog_bar=True, on_step=True)
    ...
    def validation_step(self, batch, batch_idx):
        ...
        inputs = self.get_input(batch, self.image_key)  # inputs.shape: [bs == 1, 3, 1024, 2048].  range [0 ~ 1] . bs 取决于
        reconstructions, posterior = self(inputs)  # reconstructions.shape: [bs == 1, 3, 1024, 2048].  range [0 ~ 1]

        # print(f"validation_step bs: {inputs.shape[0]}")  # add by sxh
        ...
        results = {"batch_size": inputs.shape[0], "inputs": inputs.cpu(), "reconstructions": reconstructions.cpu()}  # add by sxh
        metric_results = self.cal_batch_metric(results)  # add by sxh

        self.log_dict(log_dict_ae, prog_bar=False, logger=True, on_step=True, on_epoch=False)
        self.log_dict(log_dict_disc, prog_bar=False, logger=True, on_step=True, on_epoch=False)
        # self.log('val/ws_psnr', metric_results['total-val/ws_psnr'] / metric_results['batch_size'], prog_bar=True, logger=True, on_step=True, on_epoch=True, sync_dist=True)
        self.log('val/ws_psnr', metric_results['total-val/ws_psnr'] / metric_results['batch_size'], prog_bar=True, logger=True, on_step=True, on_epoch=True, sync_dist=True)
        return self.log_dict
        # return metric_results
    
    # def validation_step_end(self, batch_parts):
    #     # after travelsal the validation dataset
    #     pass
    #     total_num = 0
    #     total_ws_psnr = 0
    #     for i in len(batch_parts['batch_size']):
    #         total_num += batch_parts['batch_size'][i]
    #         total_ws_psnr += batch_parts['total-val/ws_psnr'][i]
    #     return {'batch_size': total_num, 'total-val/ws_psnr': total_ws_psnr}

    # def validation_epoch_end(self, validation_step_outputs):  # add by sxh
    #     # after travelsal the validation dataset
    #     total_num = 0
    #     total_ws_psnr = 0
    #     for i in range(len(validation_step_outputs)):
    #         # validation_step_outputs[i] {'batch_size': results['batch_size'], 'total-val/ws_psnr': ws_psnr}
    #         for key in validation_step_outputs[i]:
    #             if key == 'batch_size':
    #                 total_num += validation_step_outputs[i]['batch_size']
    #             elif key == 'total-val/ws_psnr':
    #                 total_ws_psnr += validation_step_outputs[i]['total-val/ws_psnr']
    #     print(f"global_step: {self.global_step}")
    #     print(f"val/ws_psnr: {total_ws_psnr / total_num : .4f}")

    #     self.log("val/ws_psnr", total_ws_psnr / total_num, prog_bar=True, logger=True)  # You can't `self.log(on_step=True)` inside `validation_epoch_end`, must be one of (False,)
    ...
    def cal_batch_metric(self, results):  # DIY function.  add by sxh
        from basicsr.metrics import my_calculate_ws_psnr, my_calculate_ws_ssim
        from basicsr.utils import img2tensor,tensor2img

        inputs = list(torch.split(results["inputs"], split_size_or_sections=1, dim=0))
        inputs = tensor2img(inputs)  # Pixel Value: [0~1] -> [0~255].  Type: numpy.ndarray.  list([C, H, W]), len(list) == batch_size
        
        reconstructions = list(torch.split(results["reconstructions"], split_size_or_sections=1, dim=0))
        reconstructions = tensor2img(reconstructions)

        assert len(inputs) == len(reconstructions)  # len == config: data.params.batch_size
        ws_psnr = 0.
        for i in range(len(inputs)):
            ws_psnr += my_calculate_ws_psnr(inputs[i], reconstructions[i], crop_border=0, input_order='HWC', test_y_channel=False)
        
        return {'batch_size': results['batch_size'], 'total-val/ws_psnr': ws_psnr}