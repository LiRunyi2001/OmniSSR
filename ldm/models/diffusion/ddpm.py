...
def range_null_space_decomposition(x0:torch.Tensor, y:torch.Tensor, H_funcs):
    # DDNM #
    # print(f"RND: x0.shape: {x0.shape}, y.shape: {y.shape}, H_funcs.ratio: {H_funcs.ratio}, H_funcs.img_dim: {H_funcs.img_dim}")
    B, C, H ,W = x0.shape
    ...
    def register_schedule(self, given_betas=None, beta_schedule="linear", timesteps=1000,
        ...
        if exists(given_betas):
            ...
            betas = make_beta_schedule(beta_schedule, timesteps, linear_start=linear_start, linear_end=linear_end,
                                       cosine_s=cosine_s)
        alphas = 1. - betas  # alphas的下标与 时间步保持一致
        alphas_cumprod = np.cumprod(alphas, axis=0)
        alphas_cumprod_prev = np.append(1., alphas_cumprod[:-1])   # alpha^bar_(t-1)
...
class LatentDiffusionSRTextWT(DDPM):
    ...
    def __init__(self,
        ...
        if not scale_by_std:
            ...
            self.register_buffer('scale_factor', torch.tensor(scale_factor))
        self.instantiate_first_stage(first_stage_config)  # SD 的 Encoder & Decoder: ldm.models.autoencoder.AutoencoderKL
        self.instantiate_cond_stage(cond_stage_config)  # SD 的 文本接口
        self.instantiate_structcond_stage(structcond_stage_config)  # StableSR 的 LR 接口
        self.cond_stage_forward = cond_stage_forward
        self.clip_denoised = False
        self.bbox_tokenizer = None
    ...
    def p_mean_variance_canvas(self, x, c, struct_cond, t, clip_denoised: bool, return_codebook_ids=False, quantize_denoised=False,
        ...
        while cur_y < x.size(-2):
            cur_y = max(grid_cols * tile_size-tile_overlap * grid_cols, 0)+tile_size  # tile_overlap: 重叠区域
            grid_cols += 1
        ...
        for row in range(grid_rows):
            ...
            for col in range(grid_cols):
                ...
                cond_tile = struct_cond[:, :, input_start_y:input_end_y, input_start_x:input_end_x]  # LR latent
                ...
                if len(input_list) == batch_size or col == grid_cols-1:
                    ...
                    if unconditional_conditioning is None or unconditional_guidance_scale == 1.:
                        struct_cond_input = self.structcond_stage_model(cond_list, t_in[:input_list.size(0)])  # Time-aware Encoder
        ...
        # img_recon = torch.clamp(img_recon, min=-1, max=1)
        # img_recon = range_null_space_decomposition(x0=img_recon, y=self.lr_erp, H_funcs=self.H_funcs)
        # x_recon = self.get_first_stage_encoding(self.encode_first_stage(img_recon))
    ...
    def p_sample_loop_canvas(self, cond, struct_cond, shape, return_intermediates=False,
        ...
        for i in iterator:  # 迭代 time step
            if time_replace is None or time_replace == 1000:
                ts = torch.full((b,), i, device=device, dtype=torch.long)
                t_replace=None
            else:
...
class LatentDiffusionSRTextWTPano(DDPM):
    """main class"""
    def __init__(self,
                 first_stage_config,
                 cond_stage_config,
    ...
    def tiled_noise_prediction(self, x, c, struct_cond, t, clip_denoised: bool, return_codebook_ids=False, quantize_denoised=False,
                        return_x0=False, score_corrector=None, corrector_kwargs=None, t_replace=None, tile_size=64, tile_overlap=32, batch_size=4, tile_weights=None,
                        unconditional_conditioning=None, unconditional_guidance_scale=None,
                        reference_sr=None, reference_lr=0.05, reference_step=1, reference_range=[100, 1000]):
        assert tile_weights is not None
    ...
    def p_mean_variance_canvas_pano(self, x, c, struct_cond, t, clip_denoised: bool, return_codebook_ids=False, quantize_denoised=False,
                        return_x0=False, score_corrector=None, corrector_kwargs=None, t_replace=None, tile_size=64, tile_overlap=32, batch_size=4, tile_weights=None, tan_weights=None,
                        unconditional_conditioning=None, unconditional_guidance_scale=None,
                        reference_sr=None, reference_lr=0.05, reference_step=1, reference_range=[100, 1000]):
        
        latent_pers = x  # [B*Np, C, H, W]
        latent_pers = rearrange(latent_pers, '(B Np) C H W -> B C H W Np', Np=self.num_tan_patch) # [B, C, H, W, Np]
        tan_weights_pers = rearrange(tan_weights, '(B Np) C H W -> B C H W Np', Np=self.num_tan_patch)

        struct_cond_pers = rearrange(struct_cond, '(B Np) C H W -> B C H W Np', Np=self.num_tan_patch)
        tile_weights_pers = rearrange(tile_weights, '(B Np) C H W -> B C H W Np', Np=self.num_tan_patch)

        noise_pers_preds = []
        for idx in range(self.num_tan_patch):
            noise_pred = self.tiled_noise_prediction(latent_pers[..., idx], c, struct_cond_pers[..., idx], t, clip_denoised, return_codebook_ids, quantize_denoised, return_x0, score_corrector, corrector_kwargs, t_replace, tile_size, tile_overlap, batch_size, tile_weights_pers[..., idx], unconditional_conditioning, unconditional_guidance_scale, reference_sr, reference_lr, reference_step, reference_range)
            noise_pers_preds.append(noise_pred)

        noise_pers = torch.stack(noise_pers_preds, dim=-1)  # [B, C, H, W, Np]
        
        model_out = rearrange(noise_pers, 'B C H W Np -> (B Np) C H W', Np=self.num_tan_patch) # rearrange to [B*Np, C, H, W]
        ...
        if self.gamma_l != 0:
            ## x_recon.shape [B*Np, C, H, W]
            old_x_recon = x_recon
            x_recon_pers = rearrange(x_recon, '(B Np) C H W -> B C H W Np', Np=self.num_tan_patch)
            img_recon_outputs = []
            
            # decoding stage #
            for idx in range(self.num_tan_patch):
                img_recon = self.decode_first_stage(x_recon_pers[..., idx])
                # img_recon = adaptive_instance_normalization(img_recon, self.init_pers[... ,idx])
                img_recon_outputs.append(img_recon)
            
            img_recon_pers = torch.stack(img_recon_outputs, dim=-1)  # [B 3 512 512 Np]  [B C H W Np]
            img_recon_pers = torch.clamp(img_recon_pers, min=-1., max=1.)
                
            ### Octadecaplex Tangent Information Interaction (OTII) start ###
            weighted_img_recon_pers = img_recon_pers * self.img_tan_weights_pers  # [B C H W Np]
            weighted_img_recon_erp = self.multi_viewer.pers2equi(weighted_img_recon_pers, fov=self.fov, nrows=self.nrows, erp_size=self.hr_erp_size, pre_upscale=self.pre_upscale, pre_mode='bicubic')
            img_tan_weights_erp = self.multi_viewer.pers2equi(self.img_tan_weights_pers, fov=self.fov, nrows=self.nrows, erp_size=self.hr_erp_size, pre_upscale=self.pre_upscale, pre_mode='bicubic')
            img_recon_erp = weighted_img_recon_erp / img_tan_weights_erp
            img_recon_erp = torch.clamp(img_recon_erp, min=-1., max=1.)

            # Gradient Decomposition (GD) in OTII, on ERP image #
            if self.gamma_e != 0:
                new_img_recon_erp = range_null_space_decomposition(x0=img_recon_erp, y=self.lr_erp, H_funcs=self.H_funcs)
                ### gamma_e: gradient direction ###
                new_img_recon_erp = (1 - self.gamma_e) * img_recon_erp + (self.gamma_e) * new_img_recon_erp
            else:
                new_img_recon_erp = img_recon_erp

            ### Octadecaplex Tangent Information Interaction (OTII) end ###
            new_img_recon_pers, _, _, _ = self.multi_viewer.equi2pers(new_img_recon_erp, fov=self.fov, nrows=self.nrows, patch_size=(img_recon_pers.shape[2], img_recon_pers.shape[3]), pre_upscale=self.pre_upscale, pre_mode='bicubic')
            new_img_recon_pers = new_img_recon_pers.cuda()
            
            # encoding stage #
            new_img_recon_pers = torch.clamp(new_img_recon_pers, min=-1, max=1)
            new_x_recon_pers = []
            for idx in range(self.num_tan_patch):
                new_x_recon = self.get_first_stage_encoding(self.encode_first_stage(new_img_recon_pers[..., idx]))
                new_x_recon_pers.append(new_x_recon)
            new_x_recon_pers = torch.stack(new_x_recon_pers, dim=-1)
            new_x_recon = rearrange(new_x_recon_pers, 'B C H W Np -> (B Np) C H W', Np=self.num_tan_patch)

            ### gamma_l interpolate ###
            x_recon = (1 - self.gamma_l) * old_x_recon + (self.gamma_l) * new_x_recon
    ...
    @torch.no_grad()
    def p_sample_canvas_pano(self, x, c, struct_cond, t, clip_denoised=False, repeat_noise=False,
                 return_codebook_ids=False, quantize_denoised=False, return_x0=False,
                 temperature=1., noise_dropout=0., score_corrector=None, corrector_kwargs=None, t_replace=None,
                 tile_size=64, tile_overlap=32, batch_size=4, tile_weights=None, tan_weights=None, 
                 unconditional_conditioning=None, unconditional_guidance_scale=None,
                 reference_sr=None, reference_lr=0.05, reference_step=1, reference_range=[100, 1000]):
        b, *_, device = *x.shape, x.device
        outputs = self.p_mean_variance_canvas_pano(x=x, c=c, struct_cond=struct_cond, t=t, clip_denoised=clip_denoised,
                                       return_codebook_ids=return_codebook_ids,
                                       quantize_denoised=quantize_denoised,
                                       return_x0=return_x0,
                                       score_corrector=score_corrector, corrector_kwargs=corrector_kwargs, t_replace=t_replace,
    ...
    def _gaussian_weights(self, tile_width, tile_height, nbatches, var=0.01):
        """Generates a gaussian mask of weights for tile contributions"""
        from numpy import pi, exp, sqrt
        import numpy as np
    ...
    @torch.no_grad()
    def p_sample_loop_canvas_pano(self, cond, struct_cond, shape, return_intermediates=False,
                      x_T=None, verbose=True, callback=None, timesteps=None, quantize_denoised=False,
                      mask=None, x0=None, img_callback=None, start_T=None,
                      log_every_t=None, time_replace=None, adain_fea=None, interfea_path=None, tile_size=64, tile_overlap=32, batch_size=4,
                      unconditional_conditioning=None, unconditional_guidance_scale=None,
        ...
        if not log_every_t:
            log_every_t = self.log_every_t
        device = self.betas.device
        b = max(batch_size, shape[0])  # add by sxh 
        # b = batch_size  # batch_size = n_samples
        if x_T is None:
            img = torch.randn(shape, device=device)
        else:
            img = x_T

        intermediates = [img]  # intermediate results at each denoising sampling
        if timesteps is None:
            timesteps = self.num_timesteps
        ...
        tile_weights = self._gaussian_weights(tile_size, tile_size, img.shape[0]).float()
        tan_weights = self._gaussian_weights(tile_width=x_T.shape[-1], tile_height=x_T.shape[-2], nbatches=img.shape[0]).float()
        init_struct_cond = struct_cond

        for i in iterator:  # iter time step
            if time_replace is None or time_replace == 1000:
                ts = torch.full((b,), i, device=device, dtype=torch.long)
                t_replace=None
            else:
                ts = torch.full((b,), i, device=device, dtype=torch.long)  # for calculate x_t -> x_(t-1)
                t_replace = repeat(torch.tensor([self.ori_timesteps[i]]), '1 -> b', b=batch_size)
                t_replace = t_replace.long().to(device)   # shape [n_samples] input of Unet
            if self.shorten_cond_schedule:
                assert self.model.conditioning_key != 'hybrid'
                tc = self.cond_ids[ts].to(cond.device)
                cond = self.q_sample(x_start=cond, t=tc, noise=torch.randn_like(cond))
            ...
            ### struct_cond not changed ###
            img = self.p_sample_canvas_pano(img, cond, struct_cond, ts,
                                clip_denoised=self.clip_denoised, return_x0=False,
                                quantize_denoised=quantize_denoised, t_replace=t_replace,
                                tile_size=tile_size, tile_overlap=tile_overlap, batch_size=batch_size, tile_weights=tile_weights,tan_weights=tan_weights,
                                unconditional_conditioning=unconditional_conditioning, unconditional_guidance_scale=unconditional_guidance_scale,
                                reference_sr=reference_sr, reference_lr=reference_lr, reference_step=reference_step, reference_range=reference_range)
    ...
    @torch.no_grad()
    def sample_canvas_pano(self, cond, struct_cond, batch_size=16, return_intermediates=False, x_T=None,
               verbose=True, timesteps=None, quantize_denoised=False,
               mask=None, x0=None, shape=None, time_replace=None, adain_fea=None, interfea_path=None, tile_size=64, tile_overlap=32, batch_size_sample=4, log_every_t=None,
               unconditional_conditioning=None, unconditional_guidance_scale=None,**kwargs):
        ...
        if cond is not None:
            if isinstance(cond, dict):
                ...
                cond = [c for c in cond] if isinstance(cond, list) else cond
        return self.p_sample_loop_canvas_pano(cond,
                                  struct_cond,
                                  shape,
                                  return_intermediates=return_intermediates, x_T=x_T,
                                  verbose=verbose, timesteps=timesteps, quantize_denoised=quantize_denoised,
